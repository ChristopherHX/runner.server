...
namespace Runner.Client
{
    partial class Program
    {
        ...
        public class Parameters {
            ...
            public bool LegacyRunner { get; set; }
            public bool LegacyServer { get; set; }
                                    ...
                                    var servertask = Task.Run(async () => {
                                        using (AnonymousPipeServerStream shutdownPipe =
                                            ...
                                            if(parameters.LegacyServer) {
                                                ...
                                                Runner.Server.Program.CreateHostBuilder(new string[0])
                                            ...
                                            if(!parameters.Quiet && line == "event: log") {
                                                ...
                                                if(!timelineRecords.TryGetValue(e.timelineId, out rec)) {
                                                    ...
                                                    if(rec.RecordId != Guid.Empty && rec.TimeLine != null && (rec.TimeLine.Count == 0 || rec.RecordId != rec.TimeLine[0].Id)) {
                                                        ...
                                                        if(parameters.Json) {
                                                            Console.WriteLine(JsonConvert.SerializeObject(new { dryrun = false, workflowName = rec.WorkflowName, job = rec.TimeLine[0].Name, jobID = rec.TimeLine[0].RefName, level = "info", matrix = JArray.Parse(jobs.FirstOrDefault(j => j.JobId == rec.TimeLine[0].Id)?.Matrix ?? "[{}]").LastOrDefault() ?? new JObject(), stepResult = ActionResultStatusConverter.FromPipelines(record.Result.Value).ToString().ToLower(), msg = $"{record.Result.Value.ToString()}: {record.Name}", stage = "Main", step = record.Name, stepID = new [] { record.Id.ToString() }, time = System.DateTime.Now }));
                                                        } else {
